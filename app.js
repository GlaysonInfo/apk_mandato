const API = "/api/v1";
const DEMO_STORAGE_KEY = "gabinete_mandato_demo_data";
...
const state = {
  ...
  teamDetail: null,
  geoMapFocusId: null,
  viewHistory: [{ view: "executivo", label: "Início" }],
  viewLabels: {
    executivo: "Início",
...
function initialDemoData() {
  ...
  return {
    ...
    overview: {
      ...
      heatmap: [
        {
          territorio_id: "territorio-centro",
        ...
        {
          territorio_id: "territorio-norte",
          territorio_nome: "Zona Norte",
          demandas: 1,
          contatos: 3,
          liderancas: 1,
          nivel_pressao: "BAIXA",
    ...
    demands: [
      {
        id: "demanda-1",
        titulo: "Iluminacao da Praca Central",
        descricao: "Moradores pedem reforco de iluminacao publica.",
        prioridade: "ALTA",
        status: "ABERTA",
        cidadao_id: "contato-1",
        cidadao_nome: "Maria Souza",
        territorio_id: "territorio-centro",
        territorio_nome: "Centro",
        responsavel_usuario_id: "user-chefe",
        responsavel_nome: "Chefe de Gabinete",
        data_abertura: now,
        created_at: now,
      },
      {
        ...
        descricao: "Solicitacao de limpeza em rua com descarte irregular.",
        prioridade: "MEDIA",
        status: "EM_TRIAGEM",
        cidadao_id: "contato-2",
        cidadao_nome: "Joao Lima",
        territorio_id: "territorio-norte",
        territorio_nome: "Zona Norte",
        responsavel_usuario_id: "user-chefe",
        responsavel_nome: "Chefe de Gabinete",
    ...
    contacts: [
      {
        id: "contato-1",
        nome: "Maria Souza",
        telefone_principal: "(21) 99999-1001",
        bairro: "Centro",
        territorio_id: "territorio-centro",
        territorio_nome: "Centro",
        nivel_relacionamento: "LIDERANCA",
        engajamento: "FORTE",
        voto_2028: "VOTO_CERTO",
        status: "ATIVO",
      },
      {
        id: "contato-2",
        nome: "Joao Lima",
        telefone_principal: "(21) 99999-1002",
        bairro: "Zona Norte",
        territorio_id: "territorio-norte",
        territorio_nome: "Zona Norte",
        nivel_relacionamento: "APOIADOR",
        engajamento: "MORNO",
      ...
      },
      {
        id: "contato-3",
        nome: "Ana Pereira",
        telefone_principal: "(21) 99999-1003",
        bairro: "Centro",
        territorio_id: "territorio-centro",
        territorio_nome: "Centro",
        nivel_relacionamento: "BENEFICIARIO",
        engajamento: "QUENTE",
        voto_2028: "VOTO_CERTO",
        status: "ATIVO",
      },
    ...
    users: [
      ...
      },
      {
        ...
        perfil: "COLABORADOR",
        email_login: "campo@gabineteia.local",
        equipe_id: "equipe-campo",
        ativo: true,
      },
    ],
    teams: [
      {
        id: "equipe-campo",
        nome: "Equipe de Campo",
        supervisor_nome: "Chefe de Gabinete",
        supervisor_usuario_id: "user-chefe",
        ativo: true,
      },
    ],
    territories: [
      { id: "territorio-centro", nome: "Centro", tipo: "REGIONAL", parent_id: null },
      { id: "territorio-norte", nome: "Zona Norte", tipo: "REGIONAL", parent_id: null },
    ],
    interactions: [],
    agenda: [
      {
        id: "agenda-1",
        titulo: "Visita a liderancas",
        tipo_agenda: "AGENDA_LIDERANCAS",
        status: "AGENDADO",
        local_texto: "Centro",
        data_inicio: now,
    ...
    sentiment: {
      positivos_percentual: 64,
      neutros_percentual: 24,
      negativos_percentual: 12,
    },
  };
}
...
function demoData() {
  try {
    ...
    console.warn("Nao foi possivel ler os dados locais do modo demo.", error);
  }
...
async function demoApi(path, options = {}) {
  ...
  if (path === "/auth/login" && method === "POST") {
    if (!body.email_login && !body.email) throw new Error("Informe o e-mail.");
    if (!body.senha) throw new Error("Informe a senha.");
    return envelope({ access_token: "demo-token", user: db.me });
  }
  if (path === "/auth/me") return envelope(db.me);
  if (path === "/political-os/overview") return envelope(db.overview);
  if (path.startsWith("/demandas") && method === "GET") return envelope(db.demands);
  if (path.startsWith("/contatos") && method === "GET") return envelope(db.contacts);
  if (path.startsWith("/usuarios")) return envelope(db.users);
  if (path.startsWith("/equipes")) return envelope(db.teams);
  if (path.startsWith("/territorios")) return envelope(db.territories);
  if (path.startsWith("/interacoes") && method === "GET") return envelope(db.interactions);
  if (path.startsWith("/agenda") && method === "GET") return envelope(db.agenda);
  if (path.startsWith("/sentimento-social")) return envelope(db.sentiment);

  if (path === "/contatos" && method === "POST") {
    const territory = db.territories.find((item) => item.id === body.territorio_id);
    const contact = {
      id: makeId("contato"),
      ...body,
      territorio_nome: territory?.nome || body.bairro || "Sem territorio",
      status: "ATIVO",
      created_at: now,
    };
    db.contacts = [contact, ...db.contacts];
    db.overview.cards.contatos = db.contacts.length;
    db.overview.cards.liderancas = db.contacts.filter((item) => item.nivel_relacionamento === "LIDERANCA" || item.eh_lideranca).length;
    saveDemoData(db);
    return envelope(contact);
  }

  if (path === "/demandas" && method === "POST") {
    const contact = db.contacts.find((item) => item.id === body.cidadao_id);
    const demand = {
      id: makeId("demanda"),
      ...body,
      status: "ABERTA",
      cidadao_nome: contact?.nome || "Sem demandante",
      territorio_id: contact?.territorio_id || body.territorio_id || "",
      territorio_nome: contact?.territorio_nome || "Sem territorio",
      responsavel_usuario_id: body.responsavel_usuario_id || db.me.id,
      responsavel_nome: db.users.find((item) => item.id === body.responsavel_usuario_id)?.nome || db.me.nome,
      data_abertura: now,
      created_at: now,
    };
    db.demands = [demand, ...db.demands];
    db.overview.cards.demandas_abertas = db.demands.filter((item) => !["CONCLUIDA", "ARQUIVADA", "CANCELADA"].includes(item.status)).length;
    saveDemoData(db);
    return envelope(demand);
  }

  if (path === "/agenda-eventos" && method === "POST") {
    const agenda = {
      id: makeId("agenda"),
      ...body,
      data_hora_inicio: body.data_inicio,
      created_at: now,
    };
    db.agenda = [agenda, ...db.agenda];
    db.overview.cards.agenda_pendente = db.agenda.filter((item) => item.status !== "REALIZADO").length;
    saveDemoData(db);
    return envelope(agenda);
  }

  if (path === "/interacoes" && method === "POST") {
    const interaction = { id: makeId("interacao"), ...body, created_at: now };
    db.interactions = [interaction, ...db.interactions];
    saveDemoData(db);
    return envelope(interaction);
  }

  if (path.includes("/demandas/") && method === "PUT") {
    const demandId = path.split("/")[2];
    db.demands = db.demands.map((item) => (item.id === demandId ? { ...item, ...body, updated_at: now } : item));
    saveDemoData(db);
    return envelope(db.demands.find((item) => item.id === demandId));
  }

  if (path.includes("/demandas/") && method === "POST") {
    const parts = path.split("/");
    const demandId = parts[2];
    const action = parts[3];
    const statusByAction = {
      assumir: "EM_ATENDIMENTO",
      concluir: "CONCLUIDA",
      reabrir: "REABERTA",
      arquivar: "ARQUIVADA",
    };
    db.demands = db.demands.map((item) => (item.id === demandId ? { ...item, status: statusByAction[action] || item.status, updated_at: now } : item));
    saveDemoData(db);
    return envelope(db.demands.find((item) => item.id === demandId));
  }

  if (path === "/auth/logout") return envelope({ ok: true });
  throw new Error("Recurso local indisponivel neste prototipo.");
}
...
function labelStatus(status) {
  const map = {
    ...
    NEUTRAL: "Estavel",
    RESTRITO: "Restrito",
    PENDENTE_IMPORTACAO: "Pendente REVISA",
  };
...
function toggleQuickContactBenefitFields() {
  const checkbox = $("#quick-contact-beneficiary-polo");
  const block = $("#quick-contact-revisa-fields");
...
function levelLabel(score) {
  if (score >= 80) return "Cadastro em alta";
  if (score >= 60) return "Cadastro estável";
  if (score >= 40) return "Cadastro sob atenção";
  return "Cadastro em alerta";
}
...
function openAgendaListInMandato({ title, eyebrow, summary, chip, meta = [], webLink, filter }) {
  const items = state.agenda
    ...
    .map((item) => ({
      ...
      tags: [item.eh_agenda_vereador ? "Prioritário" : "Equipe", labelStatus(item.status || "PLANEJADO")],
    }));
  openContextList({
    title,
    eyebrow,
...
function openAgendaRecord(agendaId) {
  ...
  openContextList({
    ...
    items: [
      {
        eyebrow: item.eh_agenda_vereador ? "Prioritário" : "Equipe",
        title: item.titulo,
          ...
          (item) => `
            <article class="stack-card">
              ...
              <div class="stack-meta">
                ...
                <div class="meta-tag">${item.eh_agenda_vereador ? "Prioritário" : "Equipe"}</div>
              </div>
            </article>
          `,
        )
...
async function createContact(event) {
  ...
  const photoFile = form.querySelector('input[name="foto"]')?.files?.[0] || null;
  if (!data.territorio_id) delete data.territorio_id;
  if (!data.polo_nome) delete data.polo_nome;
  if (!data.codigo_revisa) delete data.codigo_revisa;
  data.origem_cadastro = "MOBILE_CAMPO";
  data.eh_lideranca = form.querySelector('[name="eh_lideranca"]')?.checked || false;
  data.eh_apoiador = form.querySelector('[name="eh_apoiador"]')?.checked || false;
  data.eh_beneficiario = form.querySelector('[name="eh_beneficiario"]')?.checked || false;
  data.eh_parceria = form.querySelector('[name="eh_parceria"]')?.checked || false;
  data.beneficiario_polo = form.querySelector('[name="beneficiario_polo"]')?.checked || false;
  data.tipo_contato = data.beneficiario_polo ? "BENEFICIARIO_POLO" : data.eh_parceria ? "PARCERIA" : "CIDADAO";
  data.revisa_sync_status = data.beneficiario_polo ? "PENDENTE_IMPORTACAO" : "NAO_APLICAVEL";
  data.consentimento_registrado = true;
  data.canal_permitido = "Todos";
  if (photoFile) {
    data.foto_base64 = await fileToBase64(photoFile);
  }
  try {
    await api("/contatos", { method: "POST", body: JSON.stringify(data) });
    form.reset();
    toggleQuickContactBenefitFields();
    setMessage("#quick-contact-message", "Cadastro salvo.");
    await loadData();
...
async function createDemand(event) {
  ...
  try {
    ...
    form.reset();
    setMessage("#quick-demand-message", "Demanda salva.");
    await loadData();
    setCurrentView("operacional");
  } catch (error) {
...
async function createAgenda(event) {
  ...
  try {
    ...
    form.reset();
    setMessage("#quick-agenda-message", "Compromisso salvo.");
    await loadData();
    setCurrentView("operacional");
  } catch (error) {
...
async function createInteraction(event) {
  ...
  const form = event.currentTarget;
  const data = Object.fromEntries(new FormData(form).entries());
  data.origem_registro = "MOBILE_CAMPO";
  try {
...
async function changeDemandStatus(action) {
  ...
  try {
    if (action === "assumir") {
      ...
      await api(`/demandas/${demand.id}/concluir`, { method: "POST", body: JSON.stringify({ observacao: "Concluida pelo app mobile de campo." }) });
    } else if (action === "reabrir") {
      await api(`/demandas/${demand.id}/reabrir`, { method: "POST", body: JSON.stringify({ motivo_reabertura: "Reaberta pelo app mobile de campo." }) });
    } else if (action === "arquivar") {
      await api(`/demandas/${demand.id}/arquivar`, { method: "POST", body: JSON.stringify({ motivo: "Arquivada pelo app mobile de campo." }) });
    }
...
async function logout() {
  try {
    if (state.token) {
      await api("/auth/logout", { method: "POST", body: JSON.stringify({ origem: "mobile-campo" }) });
    }
  } catch (error) {
    console.error(error);
  } finally {
...
function bindEvents() {
  ...
  $("#quick-contact-beneficiary-polo")?.addEventListener("change", toggleQuickContactBenefitFields);
